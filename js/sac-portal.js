...
(function () {
  ...
  function renderOdeNavigationGuide() {
    ...
    container.innerHTML = `
      <p class="reference-section-desc">${ode.summary}</p>
      <div class="info-card ode-deprecation-note">
        <p><strong>Classic deprecation:</strong> ${ode.deprecation}</p>
      </div>
      <h4 class="chart-guide-subtitle">Menu path cheat sheet (use these in hands-on steps)</h4>
      <div class="table-wrap"><table class="ode-path-table"><thead><tr><th>Task</th><th>Current SAC path</th></tr></thead><tbody>${pathRows}</tbody></table></div>
      <h4 class="chart-guide-subtitle" style="margin-top:1.25rem">Story editor panels (2026)</h4>
      <div class="table-wrap"><table class="ode-path-table"><thead><tr><th>Panel</th><th>Purpose</th></tr></thead><tbody>${panelRows}</tbody></table></div>
  ...
  function renderGeoEnrichmentGuide() {
    ...
    container.innerHTML = `
      <p class="reference-section-desc">${guide.intro}</p>
      <div class="info-card ode-deprecation-note"><p>${guide.retailCoTrap}</p></div>
      ...
      <div class="table-wrap"><table class="ode-path-table"><thead><tr><th>Symptom</th><th>Cause</th><th>Fix</th></tr></thead><tbody>${trouble}</tbody></table></div>
  ...
  function renderTaskRows(d, opts) {
    ...
    return d.tasks.map((t, i) => {
      ...
      return `<div class="task-check-row${compact ? " task-check-row-compact" : ""}">
        <div class="task-check-box ${done ? "done" : ""}" data-action="toggle-task" data-key="${tk}" role="checkbox" aria-checked="${done}" tabindex="0">${done ? "✓" : ""}</div>
          ...
          <ol class="hands-on-steps">
            ${(guide.steps || []).map((step, si) => {
              ...
              <li class="hands-on-step${stepDone ? " done" : ""}">
                <div class="hands-on-step-check ${stepDone ? "done" : ""}" data-action="toggle-hands-on" data-key="${key}" role="checkbox" aria-checked="${stepDone}" tabindex="0" aria-label="Mark step ${si + 1} complete">${stepDone ? "✓" : ""}</div>
...
<header class="day-workspace-header">
  ...
  <div class="day-workspace-actions">
    <div class="day-check ${isDone ? "done" : ""}" data-action="toggle-day" data-day="${d.day}" role="checkbox" aria-checked="${isDone}" tabindex="0" aria-label="Mark day ${d.day} complete">${isDone ? "✓" : ""}</div>
...
<nav class="day-tabs" role="tablist" aria-label="Day ${d.day} sections">
  ${tabs.map((t, i) => `<button class="dtab${i === 0 ? " on" : ""}" role="tab" aria-selected="${i === 0}" data-tab="${t}" data-day="${d.day}">${tabLabels[t]}</button>`).join("")}
</nav>
</div>
<div class="day-panels">
  <div class="dpanel on lesson-with-rail" id="dp${d.day}-topics">
    <div class="lesson-main">${renderTopicsPanel(d)}</div>
    ${renderTaskRail(d)}
  </div>
  ${CONFIG.hideVideoTab ? "" : `<div class="dpanel" id="dp${d.day}-video">${renderVideoPanel(d)}</div>`}
  ${CONFIG.hideVisualTab ? "" : `<div class="dpanel" id="dp${d.day}-visual">
    <div class="img-grid">
      ${imgs.map((img) => `
  ...
  ${getHandsOnSteps(d).length ? `<div class="dpanel" id="dp${d.day}-handsOn">${renderHandsOnPanel(d)}</div>` : ""}
  <div class="dpanel" id="dp${d.day}-tasks">
    <div class="section-label">Checklist — verify these outcomes in your SAC trial</div>
    ${renderTaskRows(d)}
  </div>
  <div class="dpanel" id="dp${d.day}-resources">
  ...
  <div class="dpanel" id="dp${d.day}-ai">${renderAiPromptsPanel(d)}</div>
  <div class="dpanel" id="dp${d.day}-notes">
    <div class="section-label">Your notes for Day ${d.day}</div>
    <textarea class="notes-area" id="notes${d.day}" placeholder="${notePlaceholder}">${STATE.notes[d.day] || ""}</textarea>
  ...
  <div class="dpanel" id="dp${d.day}-case">
  ...
  function switchDTab(day, tab, btn) {
    const panel = document.getElementById("dayPanel" + day);
    if (!panel) return;
    panel.querySelectorAll(".dpanel").forEach((p) => p.classList.remove("on"));
    panel.querySelectorAll(".dtab").forEach((b) => {
      b.classList.remove("on");
      b.setAttribute("aria-selected", "false");
    });
    document.getElementById("dp" + day + "-" + tab)?.classList.add("on");
  ...
  function toggleTask(key, el) {
    ...
    panel?.querySelectorAll(`[data-action='toggle-task'][data-key="${key}"]`).forEach((box) => {
      ...
      box.setAttribute("aria-checked", isDone);
  ...
  function initReferenceNav() {
    ...
    const panels = [...nav.querySelectorAll(".reference-nav-link")]
