    ...
    2: [
      {
        ...
        steps: [
          ...
          { piece: "Room 2 — Dining room", action: "Open any sample Story", menu: "Side navigation → Stories", detail: "Open BestRunJuice or similar. Click a chart — notice Builder panel on the right.", verify: "Builder panel shows dimensions and measures — not Styling only.", clue: "Stories never store raw rows — they read from a model." },
          { piece: "Room 3 — Kitchen", action: "Open any sample Model", menu: "Side navigation → Modeler", detail: "Find Dimensions list vs Measures list. This is where RetailCo_Sales_Analytic will live from Day 3.", verify: "You can name at least one dimension and one measure you saw." },
          { piece: "Room 4 — Basement", action: "Open Files", menu: "Side navigation → Files", detail: "This is where you will upload <code>retailco_sales_analytic.csv</code> tomorrow. Download it today from Reference → Data files.", verify: "You downloaded the CSV or know where Reference tab lists it." },
          { piece: "Mini puzzle — First dish", action: "Create a practice story", menu: "Side navigation → Stories → + Create → Blank story (Canvas)", detail: "Pick any sample model from the Data Panel. Add bar chart — try drag-and-drop from Data Panel if your tenant has 2026 Q2+. Region on axis, Revenue on measure.", verify: "One bar chart renders with numbers — even if sample data.", clue: "Builder = data. Styling = colours. Data Panel = models and fields." },
          { piece: "Save your practice", action: "Save story as My_First_SAC_Story", menu: "File → Save", detail: "Day 5 replaces this with RetailCo_Executive_Dashboard on your own model.", verify: "Story appears in Side navigation → Stories list." }
        ],
    ...
    3: [
      {
        ...
        steps: [
          ...
          { piece: "Piece 3 — Start the recipe", action: "Begin import model wizard", menu: "Side navigation → Modeler → Model → Start with data → File", detail: "Select your uploaded CSV. Choose <strong>Analytic Model</strong> (not Planning).", verify: "Mapping screen shows all 31 column names." },
          ...
          { piece: "Piece 8 — Clean the edges", action: "Run Prepare Data if prompted", menu: "Transform data", detail: "Fix mixed dates to YYYY-MM-DD. Trim spaces in Region. Delete blank rows.", verify: "Preview row count still 23,760." },
          ...
          { piece: "Piece 10 — Name the model", action: "Save as RetailCo_Sales_Analytic", menu: "Model → Save as", detail: "This exact name is used in every hands-on step from Day 4 onward.", verify: "Model appears in Side navigation → Modeler with correct name." }
        ],
    ...
    4: [
      {
        ...
        steps: [
          ...
          { piece: "Lock 5a — Why Region fails", action: "Read the trap", detail: "Region = North/South/East/West/Central are sales territories — NOT mappable states. Open Reference → Geo enrichment guide.", verify: "You can explain why bar chart uses Region but map needs State or US file.", clue: "Chinnu's 'South' is not one polygon on India's map." },
          { piece: "Lock 5b — Geo Enrichment wizard", action: "Modeler → Geo Enrichment → By Area Name", menu: "Toolbar → Geo Enrichment", detail: "Country = India · Sub-Region = State column. Fix every warning in Details panel. Optional: use retailco_sales_analytic_us.csv for US trial.", verify: "Location dimension appears in model — not just Region generic dim." },
          { piece: "Lock 5c — Choropleth test", action: "Story → Geo Map → Choropleth/Drill layer", menu: "Insert → Geo Map", detail: "Location dimension + Revenue. Blank map → Save model in Modeler again.", verify: "At least one country/state shaded OR documented blockers in notes." },
    ...
    5: [
      {
        ...
        dataFile: "retailco_sales_analytic",
        steps: [
          { piece: "Frame 1 — Blank canvas", action: "Create Optimized Story on RetailCo_Sales_Analytic", menu: "Side navigation → Stories → + Create → Blank story (Canvas)", detail: "Select model RetailCo_Sales_Analytic (not sample data). Add Canvas page → name <strong>Executive Summary</strong>.", verify: "Story title bar shows your model name." },
    ...
    7: [
      {
        ...
        steps: [
          { piece: "Piece A — New object type", action: "Create Planning Model (not Analytic)", menu: "Side navigation → Modeler → Model → Start with blank model → Enable Planning", detail: "Dimensions: Account · Region · Product Category · Time · Version.", verify: "Model type says Planning in properties.", clue: "RetailCo_Sales_Analytic stays read-only — different object." },
          ...
          { piece: "Piece F — Nandan's table", action: "Create planning story with data entry table", menu: "Side navigation → Stories → Table widget", detail: "Bind RetailCo_Planning · Version = Budget · rows = accounts/regions · columns = months.", verify: "Table renders with editable cells on Budget." },
