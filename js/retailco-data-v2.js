...
(function () {
  ...
  extras.DATA_SOURCES = {
    ...
    retailco_sales_analytic_us: {
      id: "retailco_sales_analytic_us",
      name: "RetailCo US States (geo practice)",
      file: "data/retailco_sales_analytic_us.csv",
      downloadName: "retailco_sales_analytic_us.csv",
      format: "CSV · US states · geo enrichment",
      owner: "Optional Day 4 — when India State matching fails in trial tenant",
      story: "Same FMCG structure as India file but <strong>Country = United States</strong> and <strong>State = California, Texas, …</strong> for reliable <em>Geo Enrichment → By Area Name</em> in SAC trial tenants.",
      sacModel: "RetailCo_Sales_US (optional second model)",
      connectionType: "Import from file",
      grain: "3,240 rows · 10 US states × channels × brands × 12 months",
      period: "Calendar year 2025",
      firstUsedDay: 4,
      usedInDays: [4],
      columns: [
        { name: "Country", type: "Text", mapsTo: "Geo Enrichment country (United States)", chartUse: "By Area Name wizard", doNot: "" },
        { name: "State", type: "Text", mapsTo: "Geo Sub-Region — must match Supported Locations", chartUse: "Choropleth map", doNot: "Abbreviations (CA, TX) often fail" },
        { name: "US_Region", type: "Text", mapsTo: "Generic sales region (Northeast, West, …)", chartUse: "Bar chart — not map", doNot: "Not a substitute for State on maps" }
      ],
      validation: { rowCount: "3,240 rows", rule: "Geo Enrichment: Country United States + State column full names" }
    },
  ...
  extras.DAY_DATA_FILES = {
    ...
    4: ["retailco_sales_analytic", "retailco_sales_analytic_us"],
  ...
  extras.CHART_STORY_GUIDE = {
    ...
    modelFirst: [
      ...
      { step: "2", title: "Add calculations + Geo Enrichment", detail: "Day 4: Pct_vs_Target, YTD, Beverage_Revenue, then Geo Enrichment wizard (State column or US CSV) — before geo map widget." },

      { step: "3", title: "Create Optimized Story on the model", detail: "Day 5+: Side navigation → Stories → + Create → pick RetailCo_Sales_Analytic. Canvas for Chinnu (desktop). Responsive for Menon (tablet)." },
    ...
    widgets: [
      ...
      { type: "Geo map", question: "Where geographically?", dimensions: "Location dimension (after Geo Enrichment)", measures: "Revenue", retailCo: "Choropleth on State — bar chart still uses sales Region", day: 4, avoid: "Using Region (North/South) without enrichment — map stays blank" },
