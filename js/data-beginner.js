...
window.SACPortal = {
  ...
  CONFIG: {
    ...
    storageKey: "sac_beginner_state_v2",
    showCompareTab: false,
    hideVideoTab: true,
    hideVisualTab: true,
    ...
    pageTitle: "SAP Analytics Cloud Learning Portal | Absolute Beginner",
    heroTitle: "Learn SAP Analytics Cloud from zero — story by story, with RetailCo",
    heroDescription: "A gentle 14-day roadmap with detailed story lessons, official SAP documentation, copy-paste AI prompts when you are stuck, daily checklists, and the RetailCo India case study — no prior SAP experience needed.",
    brandSubtitle: "Learning Portal · Absolute Beginner Track",
  ...
  DAYS: [
    ...
    },
    {
      ...
      dayIntro: "Today you log into SAC for the first time. The interface will feel new — that is completely normal. By the end of today you will know tenant URLs, object types on the home screen, the Model → Story dependency, live vs import connection mechanics, and you will create your first chart in an Optimized Story.",
      ...
      topicsDetailed: [
        ...
        {
          title: "Home screen — Stories, Models, Connections, and more",
          story: "The SAC home screen is your launchpad. Key object types: <em>Stories</em> (dashboards), <em>Models</em> (Analytic, Planning, or Live), <em>Connections</em> (Live Data Connection or Data Import), <em>Files</em> (uploaded CSV/Excel), <em>Calendar</em> (scheduled jobs), and <em>Security</em> (users, teams, roles). Under <em>Create</em> you will see Story and Model — for this roadmap focus on Story and Analytic Model only.",
          cover: [
            "Navigate through Stories, Models, Files, Connections, and Security menus",
            "Open a sample story such as BestRunJuice if available in your tenant",
            "Identify where to create a new story versus a new model",
            "Locate System → About to confirm your SAC version in the tenant"
          ]
        },
        {
        ...
        {
          title: "Optimized Story — the story type to learn now",
          story: "SAP deprecated Classic Design for new development. <em>Optimized Story</em> uses the modern runtime with <em>Canvas</em> pages (pixel-perfect desktop layout) and <em>Responsive</em> pages (fluid grid for tablet/phone). Widgets bind to models through the Builder panel (rows/columns/measures). RetailCo's CEO dashboard = Canvas Page 1; regional managers = Responsive pages. Classic stories still exist in old tenants — do not create new ones.",
          cover: [
            "Create one blank Optimized Story in your trial tenant",
            "Add a single bar chart using sample or imported data",
            "Switch to mobile preview and observe how layout adapts",
    ...
    },
    {
      ...
      dayIntro: "A model is the foundation of everything in SAC — like the skeleton of a building. Today Menon and Nandan both send you the same sales data in different files. You import RetailCo's CSV, map columns to dimensions and measures, run Data Wrangler if needed, and validate totals against Finance's Excel before any chart gets built.",
      ...
      topicsDetailed: [
        ...
        {
          title: "Importing CSV and using Data Wrangler",
          story: "Path in SAC: <em>Create → Model → Get data from a file</em>. Upload CSV or Excel from Files. The <em>Data Wrangler</em> opens when transforms are needed — normalise date formats (Nandan's Excel often mixes DD/MM/YYYY), trim whitespace, split combined columns, remove blank rows. Map each source column to target dimension or measure on the mapping screen, preview row counts and sample values, then execute import into the embedded HANA store.",
          cover: [
            "Upload the RetailCo CSV via Create → Model → Import from file",
            "Use Data Wrangler to fix date formats and data type issues",
    ...
    },
    {
      ...
      topicsDetailed: [
        ...
        {
          title: "Geo enrichment — regions on a map of India",
          story: "<em>Geo enrichment</em> attaches geographic coordinates to dimension members. In Modeler, open the Region dimension → Location Information → enable enrichment and set location type (Region/State for Indian states). Choropleth map widgets shade regions by Revenue — South dark green at ~₹11 Cr, Central lighter. Leadership grasps geographic performance faster than a five-row table; use maps for regional overview, bar charts for precise comparisons.",
          cover: [
            "Enable geo enrichment on the Region dimension with correct location type",
            "Verify a choropleth or bubble map renders Indian regions correctly",
            "Bind Revenue measure to colour intensity on the map",
            "Understand when to use map visuals versus bar charts for regional data"
          ]
        },
        {
      ...
      tasks: [
        ...
        "Enable geo enrichment and build a choropleth map by Region",
      ...
      topicsDetailed: [
        {
          ...
          story: "Path: <em>Create → Story → Optimized Story</em> → select model <em>RetailCo_Sales_Analytic</em> → add a <em>Canvas</em> page. The canvas is a fixed grid for desktop layout — Chinnu's Monday view. The <em>Builder panel</em> (right) assigns data: chart type, dimensions to Rows/Columns, measures to Values. The <em>Styling panel</em> handles fonts, colours, and number formats — no data logic here. Insert widgets from the toolbar; drag to resize on the canvas grid.",
          cover: [
    ...
    },
    {
      ...
      topicsDetailed: [
        ...
        {
          title: "Smart Insights and Search to Insight",
          story: "<em>Smart Insights</em> scans a chart and surfaces plain-language explanations — e.g. <em>\"East revenue down 8% — Snacks fell after competitor promotion.\"</em> Enable on the region bar chart; Chinnu reads it in meetings. <em>Search to Insight</em> lets managers type questions — <em>\"Revenue by region last quarter\"</em> — and SAC builds an answer. Augmented analytics on top of your model — not a substitute for correct dimensions and measures.",
          cover: [
            "Enable Smart Insights on the regional revenue bar chart",
            "Read and interpret at least one insight bullet aloud",
            "Try Search to Insight with one RetailCo sales question",
